...
bool checkPubKeyInFile(const string& pubkeyhex) {
    int fd = open("pub/65k.txt", O_RDONLY);
    if (fd == -1) {
...
void getRandomHex(mpz_t randomNumber) {
    mpz_t range;
    mpz_init(range);
    mpz_set_ui(range, 1);
    mpz_mul_2exp(range, range, 126);
 
    mpz_t lowerBound, upperBound;
    mpz_init_set_str(lowerBound, "200000000000000000000000000000000", 16);
    mpz_init_set_str(upperBound, "3ffffffffffffffffffffffffffffffff", 16);

    mpz_t rangeLength;
    mpz_init(rangeLength);
    mpz_sub(rangeLength, upperBound, lowerBound);
    mpz_urandomm(randomNumber, state, rangeLength);
    mpz_add(randomNumber, randomNumber, lowerBound);

    mpz_clear(range);
    mpz_clear(lowerBound);
    mpz_clear(upperBound);
    mpz_clear(rangeLength);
}
...
void* threadFunction(void* arg) {
    ...
    for (int i = threadId; i < 10000000; i += numThreads) {
        Int privateKey;
        getRandomHex(privateKey.num);
